...
import java.applet.*;
/*<applet code=Scrabblet.class width=700 height=650>
</applet>*/
public class Scrabblet extends Applet implements ActionListener {
  ...
  public void start() {
    try {
	  ...
	  showStatus("connection failed"+serverName);
	  System.out.println("connection failed to "+serverName);
    }
  }
  public void stop() {
   if (!single)
  ...
  private void start_Game(int seed) {
    if (single) {
	  ...
	  System.out.println("no server found, playing solo");
      ourturn = true;
    } else {
      remove(idList);
      remove(challenge);
